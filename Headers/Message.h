...
{
public:
	OutgoingMessage(const Packet& mainPacket, const sockaddr_in& clientAddr, std::function<void(uint32_t, uint32_t)> onTimeout);
	~OutgoingMessage();
...
private:
	...
	uint32_t _timeout;
	const std::function<void(uint32_t, uint32_t)> _onTimeout;
